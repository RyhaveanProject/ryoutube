...
# ---------------- yt-dlp config (mirta-style minimal opts) ----------------
_USER_AGENTS = [
...
# Player clients ordered by bot-detection resilience (2026):
# web_safari & mweb are the most resilient on cloud IPs, ios as fallback.
_PLAYER_CLIENTS = ["web_safari", "ios", "android", "web"]
...
def _make_stream_opts():
    return {
        "quiet": True, "no_warnings": True, "skip_download": True,
        # Mirta-style simple format string — let yt-dlp pick best progressive
        # mp4. Avoid complex filters that force the video extractor into the
        # heavily bot-protected path.
...
def _stream_sync(video_id: str):
    """
    Resilient stream resolver. Tries each player_client individually until one
    succeeds. This bypasses bot detection more reliably than a single combined
    request because yt-dlp won't bail at the first failed client.
    """
    url = f"https://www.youtube.com/watch?v={video_id}"
    last_exc = None
    info = None
    for client_name in _PLAYER_CLIENTS:
        try:
            opts = {
                "quiet": True, "no_warnings": True, "skip_download": True,
                "format": "best[ext=mp4][acodec!=none][vcodec!=none]/best[acodec!=none][vcodec!=none]/best",
...
def make_token(user_id: str, device_id: str, role: str) -> str:
    payload = {
        "uid": user_id, "did": device_id, "role": role,
...
async def admin_create_user(body: CreateUserIn, _=Depends(require_admin)):
    ...
    doc = {
        "id": str(uuid.uuid4()), "email": email, "name": body.name or "",
        "password_hash": hash_password(body.password),
        "role": "user", "active": True, "device_id": None,
...
async def video_meta(video_id: str, user=Depends(get_current_user)):
    ...
    out = {
        "id": video_id,
        "title": data.get("title"), "channel": data.get("channel"),
...
async def stream_meta(video_id: str, request: Request, proxy: int = 0, user=Depends(get_current_user)):
    ...
    return {
        ...
        "stream_url": out_url, "is_live": is_live, "is_hls": is_hls,
...
async def _ensure_admin():
    ...
    if existing:
        ...
        return
    doc = {
        "id": str(uuid.uuid4()), "email": ADMIN_EMAIL.lower(), "name": "Admin",
        "password_hash": hash_password(ADMIN_PASSWORD),
        "role": "admin", "active": True, "device_id": None,
        "created_at": datetime.now(timezone.utc).isoformat(),
    }
    await db.users.insert_one(doc)
