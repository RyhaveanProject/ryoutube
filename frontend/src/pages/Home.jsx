...
export default function Home() {
  ...
  useEffect(() => {
    let alive = true;
    (async () => { if (alive) await loadFeed(); })();
    return () => { alive = false; };
  }, [loadFeed]);

  useEffect(() => {
    let alive = true;
    if (!yt.connected) { setYtRecent([]); setYtLiked([]); return; }
    (async () => {
      try {
  ...
  return (
    <PullToRefresh onRefresh={onRefresh}>
      <div className="px-3 sm:px-6 py-4" data-testid="home-page">
        {!yt.connected && yt.configured !== false && (
          <div
          ...
          >
            ...
            >
              {yt.loading ? <Loader2 className="w-4 h-4 animate-spin" /> : <LinkIcon className="w-4 h-4" />}
